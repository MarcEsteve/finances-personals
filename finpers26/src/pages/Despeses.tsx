...
const CATEGORIES = ['Habitatge', 'Alimentació', 'Transport', 'Lleure', 'Salut', 'Subscripcions', 'Professional', 'Impost', 'Altres']
...
export default function Despeses() {
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Error actualitzant despesa')
    ...
    <section>
      ...
      {transaccions.length === 0 ? (
        ...
        <>
          ...
          {filteredTransaccions.length === 0 ? (
            <p style={{ color: '#64748b', marginTop: '1rem' }}>Cap despesa amb la categoria seleccionada.</p>
          ) : (
